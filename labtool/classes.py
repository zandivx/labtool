...
class AbstractFit:
    ...
    def save(self, path: str) -> None:
        ...
        df_in_str = DataFrame(
            {"x_in": self.x_in, "y_in": self.y_in},
        ).to_string()

        df_out_str = DataFrame(
...
class CurveFit(AbstractFit):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            x, y, lambda x: f(x, *self.p), divisions, type_="CurveFit"  # array-unpacking (same as tuple unpacking)
        )
        ...
        self.df = DataFrame({"n": n, "s": s}, index=self._p_names)  # type: ignore

    def __str__(self) -> str:
        precise_df = DataFrame({"n": self.p, "s": self.u}, index=self._p_names)  # type: ignore
...
class Interpolate(AbstractFit):
    ...
    def __repr__(self) -> str:
        ret = (
            f"<Interpolate(x_in=[{self.x_in[0]:.3f}-{self.x_in[-1]:.3f}],"
...
class StudentArray:
    ...
    def __init__(
        ...
        *series,
        ...
        self.n, self.s = separate_uarray(self.array)  # type: ignore
